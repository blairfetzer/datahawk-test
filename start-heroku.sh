...
chmod a+x ./site/target/cargo/installs/apache-tomcat-7.0.55/apache-tomcat-7.0.55/bin/*.sh

#set the correct port and database settings
JAVA_OPTS="$JAVA_OPTS -XX:MaxPermSize=512M -Xmx512M -Dhttp.port=$PORT -DDATABASE_URL=$DATABASE_URL"
export JAVA_OPTS

#start Tomcat
./site/target/cargo/installs/apache-tomcat-7.0.55/apache-tomcat-7.0.55/bin/catalina.sh run
